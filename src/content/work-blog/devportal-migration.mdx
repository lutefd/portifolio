...
Step into the realm of experimentation, where headaches abound and lessons are abundant. Join me in a moment of frustration, where banging my head against the wall seemed like the only solution. This is the gripping saga of migrating a decade-old legacy product from Drupal to Next.js.
...
Everyone was excited about the idea of migrating to a modern stack. We wanted to improve the developer experience, make the developer portals faster and with a better user experience. We wanted to make it easier to build custom solutions and to integrate with other systems. We wanted to make it easier to maintain and to scale. We wanted to make it easier to deploy and to monitor. We wanted to make it easier to test and to debug. We wanted to make it easier to build and to maintain.

Oh boy, we wanted a lot of things.
...
We decided to go with Next.js. It was the most exciting option, and it would allow us to build a modern frontend with a modern stack. We could use the latest technologies and best practices. We could build a better developer experience. We could... you get the idea.

### Why Next.js?

Next.js is a great framework for building modern web applications. It's built on top of React, and it has a lot of features that make it easy to build fast, scalable, and maintainable web applications. It has a great developer experience, and it's easy to deploy and monitor — or is it?.

We got into Next.js a little bit before React Server Components (RSC) went stable, but we went with it anyway. The DX was great, and made it easier to really use Next.js, we saw a lot of potential in it. We were also excited about the idea of using a headless CMS, and we were looking into a few options. I'll skip the part where we decided to use Strapi, and jump to the honeymoon.
...
Oh the good times, it almost brings a tear to my eye.
...
It all started with a slack message from my team leader a couple of days after the presentation.

> Hey, I went through the presentation with a customer and I have a few questions about it. When can we have a meeting to discuss it?

The meeting went fine and we both left smiling. I would have the opportunity to lead this new project with that customer, which was a lot bigger than the one I mocked the migration with. I was excited, I was happy, I was... naive.

Starting off this project would be a pilot, if it all went well we would migrate other customers and the whole department would use the new stack in the future. But it had some constraints:

-   As a pilot, we would have to keep the current Drupal 7 portals running, so most of the team would have to work on it. I would be the only developer working on the new stack along with another developer that would give me a hand with the backend broker integration.
...
This client was one of our biggest client and this portal was a big deal for them, it was a secondary developer portal for a section of their operations.
...
The problems started when they started asking for previews. It's a fairly common thing for a customer to ask for a preview of the new portal before it goes live, and it's a fairly common thing for a developer to say "sure, I'll have it ready in a couple of days".
...
I have to take a step back here and say the team at SST and Open-Next is amazing, they were able to help me a **lot**, and I was able to build a preview environment in a couple of days. They're doing an amazing work to try and make Next.js a truly open and agnostic serverless framework, SST constructs are amazing and Open-Next is a great way to deploy a Next.js app on AWS. That is, if you're not using features like Suspense with streaming and ISR.

The team at Open-Next have to deal with Vercel and AWS changes and limitations, and they're doing an amazing job at it. But it's a hard task, in V2 streaming often breaks along with ISR due to changes either by Vercel or AWS. And altough the solution is evolving, and V3 seems really promising, it imposes some limitations, and that was still better than Amplify by a long shot.

The preview environment was ready, and I was able to show it to the customer. They were happy with the results, and I was happy with the results. But not with the process.

## The descent into madness

You might be thinking, "Luis, you're being a bit dramatic, it's just a preview environment". And you would be right, but it was the start of a descent into madness.

As the development went on, I started to notice a few things:

-   Next's default caching behaviour was a pain, it seemed like a blessing at first, but it was a curse. I had to deal with a lot of revalidation issues, and it was a pain to debug. I had to use a lot of workarounds to make it work, and it was a pain to maintain. In some versions the revalidation would just break, and I would have to wait for a fix or rollback to a previous version.

-   The most valuable thing that RSC would bring to the table was the ability to stream the page, and it was not stable. It was not stable at all, and it was a pain to debug. Often the connection would just hang for the timeout limit, which gave me a lot of headaches and lambdas frequently running for 1 minute, which is something you don't want to happen. So I couldn't use it, and ended up only using RSC as a data layer, which was a shame, its potential is huge.

-   The prefetch was so agressive it would often cost a lot of money. The default behaviour at that time was to prefetch all the links on the page, that include tables with hundreds of items and the like. At the time I'm writing this, the default behaviour now it's null, it's kind of funny that a boolean value defaults to null, but it's a good change. The docs say that this default amounts to this:
...
-   When server actions became stable, I was able to use it, and it was great. But it came with their own set of problems, the biggest one being the connection limit with the database. As AWS don't just spins a ton of lambdas in burst scenarios, they just create a ton of connections to the database, breaking the connection limit in a matter of seconds. So we had to migrate to Aurora Serverless just because of RDS Data API.

And with the customer wanting to have more and more complex features, that were already outside the original scope of the project, the costs were starting to get out of hand.

I spent a lot of time staring at the AWS cost monitor, fiddling with the code and talking with the maintainers of Open-Next and SST. I was able to make it work, thanks to them it was easy to deploy and monitor, but the other things? It was a pain. It was a pain to maintain, a pain to debug, a pain to test.

I was able to deliver the portal to homologation in time, and it was a success. My team leader was happy, the customer was happy but I was not happy at all.

The perfomance simply wasn't there, as the Golang backend had to send a 9MB JSON to the homepage because of a feature the customer requested, which is already an average of 4s on the wire, and the page would take sometimes 10s to render on a cold start.
...
After the portal went live, I had a few meetings with the customer to discuss the future of the portal. They were happy with the results, their Drupal one was just that bad and they wanted to migrate the other portals to the new stack. They were happy with the performance — which was actually good in pages other than the homepage — the UX, the flexibility, the possibilities.

My team leader gave the green light to migrate the other portals. And this time I had enough good sense to know we couldnt migrate the other portals like that, it was not a good idea. Not with the current state of the new stack. A few things had to change before we could migrate the other portals, and I was not willing to go through the same pain again.
...
-   The Golang backend was pretty great, but we were not using it to its full potential. Delegate most of the processing work for it and have a server side caching layer with Redis for it.
...
With Open-Next V3 most of these issues seems to be solved, with the possibilite of having a hybrid deploy, with some parts of your app in lambda and others in ECS. But it's still in beta, and we're not willing to go through the same pain again. Until it's stable, that's the path we're going to follow.
...
Most importantly I'm finally happy with the process.
