...
The perfomance simply wasn't where I wanted, as the Golang backend had to send a 9MB JSON to the homepage because of a feature the customer requested, which is already an average of 4s on the wire, and the page would take sometimes 10s to render on a cold start.
