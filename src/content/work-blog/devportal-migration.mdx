...
Sure, I might had scratched my head a bit with Next.js default caching behaviour and screamed at revalidation for a bit but it was all good. So good in fact, that I decided a tweet wouldn't do it justice. No, I had to write a **presentation** about it.
...
-   As a pilot, we would have to keep the current Drupal 7 portals running, so most of the team would have continue working on them. I would be the only developer working on the new stack with occasional help from another developer that would give me a hand with the backend broker integration.
